defmodule Rewrite.DotFormatterTest do
  ...
  describe "format/2" do
    test "formats file", context do
      in_tmp context do
        ...
        assert DotFormatter.format(DotFormatter.default()) == :ok
  ...
  describe "format_rewrite/2" do
    test "formats file", context do
      in_tmp context do
        ...
        rewrite = Rewrite.new!("**")
        assert {:ok, rewrite} = DotFormatter.format_rewrite(DotFormatter.default(), rewrite)
  ...
  describe "format_string/4" do
    ...
    test "uses identity function for none Elixir files without plugin" do
      assert DotFormatter.format_string(DotFormatter.default(), "a.some", "foo") == {:ok, "foo"}
    end
  end
  ...
  describe "format_quoted/4" do
    test "formats AST" do
      dot_formatter = DotFormatter.default()
      file = "foo.ex"
      code = "foo bar"
      formatted = "foo(bar)\n"
  ...
  describe "format_file/4" do
    test "doesn't format empty files into line breaks", context do
      in_tmp context do
        ...
        assert DotFormatter.format_file(DotFormatter.default(), "a.exs") == :ok
        assert read!("a.exs") == ""
      end
    end
    ...
    test "returns a syntax error", context do
      in_tmp context do
        ...
        assert {:error, reason} = DotFormatter.format_file(DotFormatter.default(), "a.ex")
        assert is_struct(reason, SyntaxError)
      end
    end

    test "returns an error if the file doesn't exist" do
      assert {:error, error} = DotFormatter.format_file(DotFormatter.default(), "nonexistent.exs")
  ...
  describe "format_source/4" do
    test "doesn't format empty files into line breaks", context do
      in_tmp context do
        ...
        assert {:ok, rewrite} =
                 DotFormatter.format_source(DotFormatter.default(), rewrite, "a.exs")
    ...
    test "returns an error if the file doesn't exist", context do
      in_tmp context do
        ...
        assert DotFormatter.format_source(DotFormatter.default(), rewrite, "nonexistent.exs") == {
                 :error,
                 %Rewrite.Error{
                   __exception__: true,
                   path: "nonexistent.exs",
  ...
  describe "read!/3" do
    ...
    test "raises an error when the .formatter.exs has no config", context do
      in_tmp context do
        write!(".formatter.exs": "")
        message = ~s|The file ".formatter.exs" does not contain a valid formatter config.|
    ...
    test "raises an error when the .formatter.exs is invalid", context do
      in_tmp context do
        write!(".formatter.exs": ":oops")
        message = ~s|The file ".formatter.exs" does not contain a valid formatter config.|
  ...
  describe "inputs/1" do
    test "returns inputs", context do
      in_tmp context do
        write!(
          ".formatter.exs": """
          [inputs: "a.ex"]
        ...
        assert DotFormatter.inputs(dot_formatter) == [~g|a.ex|d]
      end
    end

    test "returns inputs from all .formatter.exs", context do
      in_tmp context do
        write!(
          ".formatter.exs": """
          [
            inputs: ["a.ex", "foo/**/*.ex"],
            subdirectories: ["bar", "baz"]
          ]
          """,
          "bar/.formatter.exs": """
          [
        ...
        assert DotFormatter.inputs(dot_formatter) ==
                 [~g|foo/**/*.ex|d, ~g|a.ex|d, ~g|baz/**/*.json|d, ~g|bar/**/*.heex|d]
      end
    end
  end
end
